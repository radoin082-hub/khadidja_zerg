...
public class MainActivityTabTime extends AppCompatActivity {
    ...
    private void downLoadJsonDataTimeTable(int id_spec, int id_niv, int id_sec, int id_grp) {
        ...
        requestQueueForeground.add(new JsonArrayRequest(Request.Method.GET
                , "https://num.univ-biskra.dz/psp/emploi/section2_public?select_spec=" + id_spec + "&niveau=" + id_niv + "&section=" + id_sec + "&groupe=" + (id_grp == 0 ? "null" : id_grp) + "&sg=0&langu=" + (SimilarParts.getLanguageStatus() ? "fr" : "ar") + "&sem=2&id_year=2"
                , null
                , this::handleJsonResponseForeground
                , this::handleJsonError));
        requestQueueBackground.add(new JsonArrayRequest(Request.Method.GET
                , "https://num.univ-biskra.dz/psp/emploi/section2_public?select_spec=" + id_spec + "&niveau=" + id_niv + "&section=" + id_sec + "&groupe=" + (id_grp == 0 ? "null" : id_grp) + "&sg=0&langu=" + (!SimilarParts.getLanguageStatus() ? "fr" : "ar") + "&sem=2&id_year=2"
                , null
                , response -> handleJsonResponse(response, (!SimilarParts.getLanguageStatus() ? timeTableArrayListFr : timeTableArrayListAr))
                , this::handleJsonError));
    }
    ...
    private void handleJsonResponse(JSONArray response, ArrayList<TimeTable> timeTableArrayList) {
        for (int i = 0; i < response.length(); i++) {
            ...
            TimeTable timeTable = new TimeTable(
                    jsonArray.optString(0) // className
                    ...
                    , (jsonArray.optString(19).equals("1"))
                    , (jsonArray.optString(20).equals("1"))
    ...
    private ArrayList<TimeTable> getListOfDay(int id) {
        ...
        for (TimeTable timeTable : getCurrentList())
            if (timeTable.getDayOfWeek() == chosenDay)
                listChosenDay.add(timeTable);
    ...
    private void createSession(TimeTable session) {
        ...
        sessionLayout.setLayoutParams(new LinearLayout.LayoutParams(
        ...
        leftLayout.setLayoutParams(new LinearLayout.LayoutParams(
                (int) (230 * getResources().getDisplayMetrics().density), // Width
                LinearLayout.LayoutParams.MATCH_PARENT
        ));
        ...
        moduleName.setLayoutParams(new LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.MATCH_PARENT,
        ...
        profName.setLayoutParams(new LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.MATCH_PARENT,
                LinearLayout.LayoutParams.WRAP_CONTENT
        ));
        ...
        moduleShortcut.setLayoutParams(new LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.MATCH_PARENT,
                LinearLayout.LayoutParams.WRAP_CONTENT
        ));
    ...
    private void openPlace(TimeTable session) {
        if (session.isOnline()) { // open meet
            if (!session.getOnlineLink().isEmpty()) {
                ...
                Toast.makeText(MainActivityTabTime.this, "URL Is Empty", Toast.LENGTH_SHORT).show();
        } else {   // Open GPS
